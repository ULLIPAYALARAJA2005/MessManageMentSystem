...
def book_meal(current_user):
    ...
    if menu:
        ...
        if "T" in menu.get('deadline', ''):
            # Full datetime format: "2026-04-11T19:00"
            deadline_datetime = datetime.datetime.strptime(menu['deadline'], "%Y-%m-%dT%H:%M")
        else:
            # Simple time format: "19:00" — deadline is on the SAME day as the menu date
            deadline_time = datetime.datetime.strptime(menu.get('deadline', '23:59'), "%H:%M").time()
            deadline_datetime = datetime.datetime.combine(
                datetime.datetime.strptime(date_str, "%Y-%m-%d").date(), 
                deadline_time
            )
            # NOTE: No day subtraction — the deadline is on the booking date itself
...
def handle_complaints(current_user):
    if request.method == 'GET':
        complaints = list(db.complaints.find({"status": {"$ne": "Deleted by Student"}}).sort("createdAt", -1))
...
@student_bp.route('/complaints/<id>', methods=['PUT', 'DELETE'])
@token_required(['Student'])
def edit_or_delete_complaint(current_user, id):
    complaint = db.complaints.find_one({"_id": ObjectId(id)})
    if not complaint:
        return jsonify({"message": "Not found"}), 404
    if complaint.get('studentId') != str(current_user['_id']):
        return jsonify({"message": "Only the owner can modify this complaint"}), 403
        
    if request.method == 'DELETE':
        db.complaints.update_one({"_id": ObjectId(id)}, {"$set": {"status": "Deleted by Student"}})
...
@student_bp.route('/auto-order', methods=['GET'])
@token_required(['Student'])
def get_auto_order_settings(current_user):
    config = current_user.get('autoConfig', {})
    if not config:
        config = {
            "enabled": False,
            "autoPay": False,
...
@student_bp.route('/auto-order', methods=['POST'])
@token_required(['Student'])
def update_auto_order_settings(current_user):
    data = request.json
    ...
    config = {
        ...
        "autoPay": autoPay,
        "plan": plan,
        "lastRunDate": current_user.get('autoConfig', {}).get('lastRunDate', '')
    }
    
    db.users.update_one({"_id": current_user['_id']}, {"$set": {"autoConfig": config}})
    return jsonify({"message": "Auto-order settings saved successfully", "config": config}), 200

@student_bp.route('/notifications', methods=['GET'])
@token_required(['Student'])
def get_notifications(current_user):
    notifs = list(db.notifications.find({"studentId": str(current_user['_id'])}).sort("createdAt", -1).limit(20))
    for n in notifs:
        n['_id'] = str(n['_id'])
