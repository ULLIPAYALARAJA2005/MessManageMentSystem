...
def verify_student(current_user):
    ...
    if not student_id or not secret_code or not section:
        return jsonify({"message": "Student ID, Secret Code and Section required"}), 400
    ...
    correct_code = booking.get('codes', {}).get(actual_meal_key)
    if not correct_code or str(correct_code).strip() != str(secret_code).strip():
...
def get_daily_stats(current_user):
    ...
    meal_stats = {
        "Morning Tea": {"total": 0, "completed": 0, "pending": 0},
        "Tiffin": {"total": 0, "completed": 0, "pending": 0},
        "Lunch": {"total": 0, "completed": 0, "pending": 0},
        "Evening Tea": {"total": 0, "completed": 0, "pending": 0},
        "Evening Snacks": {"total": 0, "completed": 0, "pending": 0},
        "Dinner": {"total": 0, "completed": 0, "pending": 0}
    }
